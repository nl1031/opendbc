...
static void subaru_rx_hook(const CANPacket_t *msg) {
  ...
  } else if ((msg->addr == MSG_SUBARU_Steering_Torque) && (msg->bus == SUBARU_MAIN_BUS)) {
    int angle_meas_new = (GET_BYTES(msg, 4, 2) & 0xFFFFU);
    // convert Steering_Torque -> Steering_Angle to centidegrees
    angle_meas_new = ROUND(to_signed(angle_meas_new, 16) * -2.17);
    update_sample(&angle_meas, angle_meas_new);
  }

  // driver torque (also present on angle cars)
...
static bool subaru_tx_hook(const CANPacket_t *msg) {
  ...
  // Match openpilot_justin/outback-23 panda safety: allow ~1 deg per TX at all
  // speeds. JacobW-style 0.15 deg@highway rejects carcontroller's 1 deg/step,
  // drops ES_LKAS_ANGLE while relay blocks stock → EPS/LKAS fault on small
  // corrections or light driver input.
  ...
  // steer cmd checks (angle LKAS — Outback 2023 etc.)
  if (subaru_lkas_angle) {
...
static safety_config subaru_init(uint16_t param) {
  ...
  // Experimental Outback 2023 / LKAS_ANGLE — enabled outside ALLOW_DEBUG for this fork
